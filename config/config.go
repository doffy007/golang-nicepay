...
func Init() {
	...
	// NICEPAY MerchantID
	if ConfigureApp.MerchantId = viper.GetString("NICEPAY_MERCHANT_ID"); ConfigureApp.MerchantId == "" {
		log.Fatalln("[INIT] NICEPAY_MERCHANT_ID is missing in .env")
	}

	// NICEPAY MerchantKey
	if ConfigureApp.MerchantKey = viper.GetString("NICEPAY_MERCHANT_KEY"); ConfigureApp.MerchantKey == "" {
		log.Fatalln("[INIT] DB_USERNAME is missing in .env")
	}
